...
The `http` transport now supports stateless operation when every request includes both of these headers:
...
- The server reuses a stateless transport keyed by the `x-argocd-base-url` and `x-argocd-api-token` header values.
- If those headers are omitted, the HTTP transport continues to use the existing session-based behavior with `mcp-session-id`.
