  ...
  constructor(serverInfo: ServerInfo) {
    ...
    this.addJsonOutputTool(
      ...
      async ({ applicationSet }, client) =>
        await client.generateApplicationSet({
          applicationSet
        } as V1alpha1ApplicationSetGenerateRequest)
    );
    ...
    if (!isReadOnly) {
      ...
      this.addJsonOutputTool(
        ...
        async ({ applicationSet, applicationSetNamespace }, client) => {
          const options: Record<string, string> = {};
          if (applicationSetNamespace) options.appsetNamespace = applicationSetNamespace;
          return await client.updateApplicationSet(
            applicationSet as V1alpha1ApplicationSet,
            options
          );
        }
      );
      ...
      this.addJsonOutputTool(
        ...
        async (
          { applicationSetName, applicationSetNamespace, cascade, propagationPolicy },
          client
        ) => {
          const options: Record<string, string | boolean> = {};
          if (applicationSetNamespace) options.appsetNamespace = applicationSetNamespace;
